// ==============================================================
// DEMO catalog logic — sama macam catalog.js sebenar,
// tapi guna DEMO_SETTINGS / DEMO_CATEGORIES / DEMO_PRODUCTS (tiada Supabase)
// ==============================================================

let state = {
  settings: DEMO_SETTINGS,
  categories: DEMO_CATEGORIES,
  products: DEMO_PRODUCTS.filter(p => p.stock_status !== "HIDDEN" && p.is_visible),
...
function applySettingsToDOM() {
  ...
  document.title = `DEMO — ${s.store_name}`;
  ...
  [["Instagram", s.instagram], ["Facebook", s.facebook], ["TikTok", s.tiktok], ["Google Maps", s.google_maps]]
    .forEach(([label, url]) => {
      if (url) {
  ...
  if (s.enable_whatsapp_button && s.whatsapp) {
    ...
    fab.href = whatsappLink(s.whatsapp, `Hi ${s.store_name}, saya berminat untuk order.`);
    fab.target = "_blank";
  }
}
...
function renderCategoryChips() {
  ...
  all.forEach(name => {
    ...
    chip.onclick = () => { state.activeCategory = name; renderCategoryChips(); renderGrid(); };
    wrap.appendChild(chip);
  });
}

function renderSpecials() {
  const specials = state.products.filter(p => p.is_featured);
...
function openModal(p) {
  ...
  if (state.settings.enable_whatsapp_button && p.stock_status !== "SOLD_OUT") {
    ...
    waBtn.href = whatsappLink(state.settings.whatsapp, `Hi ${state.settings.store_name}, saya berminat untuk order ${p.name}.`);
